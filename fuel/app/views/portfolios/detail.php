<!DOCTYPE html>
<html>

<head>
  <meta charset="utf-8">
  <title>Portfolios-Detail</title>
  <?= Asset::css('detail.css'); ?>
</head>

<body>

  <?php
  function h($s)
  {
    return htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8');
  }
  $img = $p['screenshot_url'] ?: $p['placeholder'];
  $is_github = !empty($p['source_code_url']) && preg_match('~github\.com~i', $p['source_code_url']);
  ?>
  <div class="pf-detail-card">
    <div class="pf-hero">
      <img src="<?= h($img) ?>" alt="<?= h($p['title']) ?>">
      <?php if (!empty($p['project_url'])): ?>
        <a class="pf-link pf-project-url" href="<?= h($p['project_url']) ?>" target="_blank" rel="noopener"><?= h($p['project_url']) ?></a>
      <?php endif; ?>
    </div>

    <div class="pf-meta">
      <div class="pf-row">
        <div class="pf-label">タイトル</div>
        <div class="pf-value"><?= h($p['title']) ?></div>
      </div>

      <div class="pf-row">
        <div class="pf-label">説明</div>
        <div class="pf-value"><?= nl2br(h($p['description'])) ?></div>
      </div>

      <?php if (!empty($p['tags']) || !empty($p['source_code_url'])): ?>
        <div class="pf-tags">
          <?php if (!empty($p['tags'])): ?>
            <?php foreach ($p['tags'] as $tag): ?>
              <span class="pf-tag"><?= h($tag) ?></span>
            <?php endforeach; ?>
          <?php endif; ?>

          <?php if (!empty($p['source_code_url'])): ?>
            <a class="pf-fab" href="<?= h($p['source_code_url']) ?>" target="_blank" rel="noopener" title="ソースコード">
              <?php if ($is_github): ?>
                <!-- GitHubの簡易SVGアイコン -->
                <svg viewBox="0 0 16 16" width="24" height="24" aria-hidden="true">
                  <path fill="currentColor"
                    d="M8 0C3.58 0 0 3.58 0 8a8 8 0 0 0 5.47 7.59c.4.07.55-.17.55-.38
                   0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13
                   -.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66
                   .07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15
                   -.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27
                   c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15
                   0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.19
                   0 .21.15.46.55.38A8 8 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                </svg>
              <?php else: ?>
                <span style="font-weight:700;">Code</span>
              <?php endif; ?>
            </a>
          <?php endif; ?>
        </div>
      <?php endif; ?>
    </div>
  </div>
</body>

</html>
